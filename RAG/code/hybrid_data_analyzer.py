...
class HybridDataAnalyzer:
    ...
    def __init__(self):
        ...
        print("🔧 HybridDataAnalyzer 초기화 완료")
    ...
    def save_analysis_result(self, result: str, filename: str = None) -> str:
        ...
        try:
            ...
            analysis_data = {
                ...
                "analysis_method": "Hybrid Vector-based Analysis (LlamaIndex + CLOVA)",
                "result": result
            }
    ...
    def run_analysis(self, rebuild_vectors: bool = False) -> bool:
        """전체 분석 프로세스 실행"""
        print("=" * 60)
        print("🚀 하이브리드 벡터 기반 데이터 분석 시스템")
        print("📋 LlamaIndex 문서 처리 + CLOVA 벡터 저장")
        print("=" * 60)
        
        # 1. 벡터 처리 (LlamaIndex 방식 + CLOVA 저장)
        print("\n1️⃣ 하이브리드 벡터 처리 중...")
        vector_success = self.vector_manager.process_documents(rebuild=rebuild_vectors)
        
        if not vector_success:
            print("❌ 하이브리드 벡터 처리 실패")
            return False
...
def main():
    """메인 실행 함수"""
    analyzer = HybridDataAnalyzer()
    success = analyzer.run_analysis(rebuild_vectors=True)
    
    if success:
        print("\n✅ 전체 하이브리드 벡터 기반 분석 프로세스 완료!")
    else:
        print("\n❌ 하이브리드 벡터 기반 분석 프로세스 실패!")
