#!/usr/bin/env python3
"""
CLOVA와 호환되는 벡터 관리 시스템
- LlamaIndex의 데이터 처리 파이프라인 유지
- CLOVA 세그멘테이션 API로 효율적인 청킹
- CLOVA 임베딩 API로 CLOVA 호환 벡터 생성
"""
...
from clova_embedding import ClovaEmbeddingClient
from clova_segmentation import ClovaSegmentationClient


class ClovaVectorManager:
    """CLOVA와 호환되는 벡터 관리 시스템"""
    
    def __init__(self, data_dir: str = "/Users/Chris/Desktop/JH/MiraeassetNaver/RAG/data"):
        self.data_dir = Path(data_dir)
        self.vector_dir = Path("/Users/Chris/Desktop/JH/MiraeassetNaver/RAG/vector_db_clova")
        self.vector_dir.mkdir(exist_ok=True)
        
        # CLOVA 클라이언트 초기화
        self.embedding_client = ClovaEmbeddingClient()
        self.segmentation_client = ClovaSegmentationClient()
        
        # 벡터 저장소
        self.vectors_file = self.vector_dir / "clova_vectors.pkl"
        self.metadata_file = self.vector_dir / "clova_metadata.json"
    ...
    def _process_csv_files(self) -> bool:
        ...
        for csv_file in csv_files:
            try:
                ...
                # CLOVA 세그멘테이션 API로 청킹 (llamaindex_rag_system과 동일한 파라미터)
                print(f"  🔧 CSV 데이터 - 최적화된 세그멘테이션 적용")
                chunks = self._segment_text_with_clova(text_content, max_length=2048)
                
                # 각 청크를 CLOVA 임베딩 API로 벡터화
                for i, chunk in enumerate(chunks):
                    vector = self.embedding_client.get_embedding(chunk)
    ...
    def _dataframe_to_text(self, df: pd.DataFrame, filename: str) -> str:
        """DataFrame을 텍스트로 변환 (llamaindex_rag_system과 동일)"""
        text_parts = []
    ...
    def _article_to_text(self, article: Dict, index: int) -> str:
        """뉴스 기사를 텍스트로 변환 (llamaindex_rag_system과 동일)"""
        text_parts = []
    ...
    def _segment_text_with_clova(self, text: str, max_length: int = 512) -> List[str]:
        """CLOVA 세그멘테이션 API로 텍스트를 청크로 분할 (llamaindex_rag_system과 동일)"""
        try:
            # CLOVA 세그멘테이션 API 사용 (llamaindex_rag_system과 동일한 파라미터)
        ...
        # 폴백: 간단한 청킹 (llamaindex_rag_system과 동일)
