...
class SecuritiesKRXAPI:
    def __init__(self):
        # 유가증권 API 엔드포인트 (스펙에 맞게)
        self.base_urls = [
    ...
    def get_securities_daily_trading_data(self, target_date):
        """
        특정 날짜의 유가증권 일일 매매 정보를 가져옵니다.
        
        Args:
            target_date (str): 'YYYYMMDD' 형식의 날짜
            
        Returns:
            pandas.DataFrame: 유가증권 일일 매매 데이터
        """
        # 유가증권 일일 매매 정보 API 파라미터 (스펙에 따라 basDd 사용)
        params = {
            'basDd': target_date
        }
        
        try:
            print(f"{target_date} 유가증권 일일 매매 데이터를 가져오는 중...")
            
            # 현재 URL 사용
            current_url = self.base_urls[self.current_url_index]
            print(f"사용 중인 URL: {current_url}")
            ...
            if 'OutBlock_1' in data:
                ...
                print(f"데이터 가져오기 성공: {len(df)}개 유가증권")
                return df
    ...
    def get_securities_daily_trading_data_alternative(self, target_date):
        """
        대체 방법: POST 요청으로 유가증권 일일 매매 정보를 가져옵니다.
        """
        ...
        try:
            print(f"{target_date} 유가증권 일일 매매 데이터를 가져오는 중... (POST 방법)")
            
            # 현재 URL 사용
            current_url = self.base_urls[self.current_url_index]
            print(f"POST 사용 중인 URL: {current_url}")
...
def main():
    # 유가증권 KRX API 인스턴스 생성
    securities_api = SecuritiesKRXAPI()
    
    # 2024년 7월 22일 데이터 요청 (과거 날짜로 테스트)
    target_date = "20240722"
    
    print("=" * 50)
    print("KRX 유가증권 일일 매매 정보 수집")
    print("=" * 50)
    
    # 여러 URL을 시도
    df = pd.DataFrame()
    
    for i in range(len(securities_api.base_urls)):
        securities_api.current_url_index = i
        print(f"\n=== URL {i+1} 시도 중 ===")
        
        # GET 요청 시도
        df = securities_api.get_securities_daily_trading_data(target_date)
        ...
        # POST 요청 시도
        if df.empty:
            print("GET 방법 실패, POST 방법 시도...")
            df = securities_api.get_securities_daily_trading_data_alternative(target_date)
    ...
    if not df.empty:
        ...
        filename = os.path.join(data_dir, f"krx_securities_daily_trading_{target_date}.csv")
        ...
        print("다음 사항을 확인해주세요:")
        print("1. 인터넷 연결 상태")
        print("2. KRX 유가증권 API 접근 가능 여부")
        print("3. 요청한 날짜에 유가증권 거래 데이터 존재 여부")
